...
namespace Piccolo
{
    bool find_component_field(std::weak_ptr<GObject> game_object, const char* field_name,
                              Reflection::FieldAccessor& field_accessor, void*& target_instance)
    {
        ...
        // 找到对应的component
        auto component_iter = std::find_if(components.begin(), components.end(), [current_name](auto c) {
        ...
        if (component_iter != components.end()) {
            ...
            while (std::getline(iss, current_name, '.')) {
                ...
                field_instance = field_accessor.get(field_instance);
    ...
    bool LuaComponent::loadScript(const std::string& script)
    {
        m_lua_script = script;
        return true;
    }
